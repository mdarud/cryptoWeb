...
$(".method-select-file").on("change", function () {
  if (this.value == "affine") {
    $(".file-input").html(
      '<div class="col-md-6 form-group mb-3"><label for="" class="col-form-label">Key a</label> <input type="number" class="form-control" name="keyA" id="keyA" placeholder="Insert key a"/></div><div class="col-md-6 form-group mb-3"><label for="" class="col-form-label">Key b</label> <input type="number" class="form-control" name="keyB" id="keyB" placeholder="Insert key b"/></div>'
    );
  } else {
    $(".file-input").html(
      '<div class="col-md-12 form-group mb-3"><label for="" class="col-form-label">Key</label> <input type="text" class="form-control" name="key" id="key" placeholder="Insert key"/></div>'
    );
  }
});
...
$("#plug").on("change", function () {
  pattern = /^(?:([A-Z])(?!.*\1))*$/;
  if (!pattern.test(this.value) || this.value.length % 2 == 1) {
    this.style.color = "red";
    $("#encryptEnigma").prop("disabled", true);
  } else {
    this.style.color = "black";
    $("#encryptEnigma").prop("disabled", false);
  }
});
